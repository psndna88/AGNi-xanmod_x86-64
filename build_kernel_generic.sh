...
if ([ -f $KERNELDIR/../linux-headers*.deb ] && [ -f $KERNELDIR/../linux-image*.deb ] && [ -f $KERNELDIR/../linux-libc*.deb ]); then
	...
	makeself --gzip --threads 8 --needroot --nomd5 --nocrc --quiet $KERNELDIR/DEB_TEMP/ AGNi-xanmod-kernel-$AGNI_VERSION-debian-$AGNI_KERNEL_LINUX-$AGNI_BUILD_TYPE.run AGNi_kernel_x86-64 ./install_agni.sh
	rm -rf $KERNELDIR/DEB_TEMP 2>/dev/null
